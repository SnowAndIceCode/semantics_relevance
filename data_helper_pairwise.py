...
'''
    v1:mask 有些问题 {稍后对比huggfaceing 库实现}
'''
...
class PairwiseDataset(Dataset):
    ...
    def load_data(self,data_file):
        ...
        for i in tqdm(range(len(data_df))):
            ...
            #     break
            query = data_df['query'].iloc[i]
            doc_neg = data_df['doc_neg'].iloc[i]
            doc_posi = data_df['doc_posi'].iloc[i]

            datas.append({"query": query, "doc_neg": doc_neg, "doc_posi": doc_posi})
    ...
    def __getitem__(self, idx):
        sample = self.data[idx]
        query = sample["query"]
        doc_neg = sample["doc_neg"]
        doc_posi = sample["doc_posi"]
        ...
        neg_token_type_ids = neg_encoding["token_type_ids"].squeeze(0)
        posi_input_ids = posi_encoding["input_ids"].squeeze(0)
        posi_attention_mask = posi_encoding["attention_mask"].squeeze(0)
        posi_token_type_ids = posi_encoding["token_type_ids"].squeeze(0)

        # Mask 部分单词用于 MLM 任务
        if self.train_type == 'post_pretrain':
            neg_input_ids, neg_labels = self.mask_tokens(neg_input_ids)
            posi_input_ids, posi_labels = self.mask_tokens(posi_input_ids)
            return neg_input_ids,posi_input_ids,neg_attention_mask,posi_attention_mask, neg_token_type_ids,posi_token_type_ids,neg_labels,posi_labels
        return neg_input_ids,posi_input_ids,neg_attention_mask,posi_attention_mask, neg_token_type_ids,posi_token_type_ids
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return input_ids, attention_mask, token_type_ids, torch.tensor(is_next,dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    # train_data_path = 'dataset/skip_above_llm_filter_pairwise.csv'
    # test_data_path = 'dataset/21Q4-22Q1Q2Q3-23Q1Q2Q3-evaled-all_process_new_dup_recorrect.csv'
    dataset = CustomDataset(train_data_path, tokenizer, 64,train_type='finetuing')
    # dataset = PairwiseDataset(train_data_path, tokenizer, 64,train_type='finetuing')
    # dataset = PairwiseDataset(train_data_path, tokenizer, 64)
