...
from modeling import PostTrainModel
...
from criteria import TripleLoss,DoubleLoss
import pandas as pd
...
def load_model(args):
    if args.train_type == 'finetune':
        model = AutoModelForSequenceClassification.from_pretrained(args.pretrain_path,ignore_mismatched_sizes=True)
    elif args.train_type == 'post_pretrain':
        config = BertConfig.from_pretrained(args.pretrain_path)
        model = PostTrainModel(config,args.num_layer)
    ...
    for epoch in range(1, args.num_train_epochs + 1):
        for step, batch in enumerate(train_dataloader, start=1):
            ...
            if global_step % args.logging_steps == 0:
                ...
                logger.info("global step: %d, epoch: %d, batch: %d, total_loss: %.4f, mlm_loss: %.4f, pointwise_loss: %.4f,time cost: %.2fs" %
            ...
            with open(args.log_path,'a+') as fw:
                    ...
                    if args.test_file:
                        ...
                        if args.save_best:
                            if best_metrics < auc:
                                ...
                                fw.write(f"epoch:{epoch}--global_step:{global_step}--"+"bestmodel: %.4f" % (auc)+'\n')
                                logging.info("Saving model")
                                torch.save(model_to_save.state_dict(), os.path.join(args.output_dir, "best_"+WEIGHTS_NAME))
                ...
                if global_step % step_per_epoch == 0: # 每个epoch结束
                    ...
                    torch.save(model_to_save.state_dict(), os.path.join(args.output_dir, f"epoch-{epoch}-"+WEIGHTS_NAME))
...
def evaluate(args,model, dataloader, device):
    ...
    if args.mode == 'predict':
        for sessionid,infoid,query,doc,pred,label in zip(all_sessionid,all_infoid,all_query,all_doc,preds,labels_array):
            all_pred_result.append({'sessionid':sessionid,'infoid':infoid,'query':query,'doc':doc,'pred':pred,'label':label})
